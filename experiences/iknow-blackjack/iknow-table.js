/**
 * IKnowTable -- Room 4's equipment dock for the "I Know Blackjack" task: just
 * a floating screen directly in front of the fly, with a canvas-textured
 * display showing the current hand, the fly's decision, and the live
 * Q-values (no desk, no stand -- unnecessary set dressing). This is a plain
 * `Station` (framework base class), so it plugs into the same `addStation()`
 * / label / lifecycle machinery as any other station -- only the drawing on
 * its screen is task-specific. Standalone copy for iknow-blackjack -- not
 * shared with experiences/blackjack/'s card-table.js.
 *
 * Sized to sit in front of the untethered platform (see iknow-rig.js).
 */
...
export class IKnowTable extends Station {
  constructor(opts = {}) {
    super({
      name: 'Skill Card Table', kickRadius: 0, collisionRadius: 0, // decorative; not walked to
      label: 'I KNOW BLACKJACK', sublabel: 'downloaded Q-readout -> DNa01/DNp03/DNp13',
      labelColor: '#e8e0f5',
      ...opts,
    });
  }
